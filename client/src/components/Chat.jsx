...
import { playCrystalBreak, playMessageNotification } from '../utils/sounds';
import {
  FiSend,
  FiImage,
  FiMic,
...
const Chat = () => {
  ...
  const { initiateCall, CALL_STATES, callState } = useCall();
  ...
  const isInitialLoad = useRef(true); // Track if conversation is opening for the first time
  ...
  useEffect(() => {
    if (conversationId) {
      isInitialLoad.current = true; // Reset scroll mode on conversation change
      fetchConversation();
      fetchMessages();
    }
  }, [conversationId]);
  ...
  useEffect(() => {
    if (messages.length > 0) {
      if (isInitialLoad.current) {
        scrollToBottom('auto'); // Instant scroll on entry
        isInitialLoad.current = false;
      } else {
        scrollToBottom('smooth'); // Smooth scroll for live incoming/outgoing messages
      }
    }
  }, [messages]);
      ...
      if (message.sender._id !== user.id) {
        ...
        if (
          ...
          new Notification('New message', {
            ...
            icon:
              message.sender.profilePicture || '/logo.png'
          });
        }
      }
    }
                ...
                <div className="space-y-3">
                  {conversations
                    .map((conv, idx) => {
                      ...
                      if (!otherUser) {
                        console.warn(
                          'Could not find other user in conversation:',
                          conv._id
                        );
                      ...
                      return (
                        <motion.button
                          ...
                          className={`w-full rounded-2xl p-4 lg:p-6 shadow-lg hover:shadow-xl transition-all hover:scale-105 active:scale-95 group ${
                            hasUnread
                        ...
                        >
                          <div className="flex items-center space-x-4">
                            ...
                            <div className="flex-1 text-left min-w-0">
                              <h3
                                className={`text-lg truncate group-hover:text-primary transition ${
                                  hasUnread
                            ...
                            <div className="flex-shrink-0 flex items-center space-x-3">
                              {hasUnread && (
                                <div className="bg-primary text-white text-sm font-bold rounded-full w-8 h-8 flex items-center justify-center shadow-lg">
                                  {conv.unreadCount >
                                  9
                                    ? '9+'
                                    : conv.unreadCount}
      ...
      <AnimatePresence>
        {specialExperience && (
          <SpecialMessageExperience
            type={
              specialExperience.type
            }
            senderName={
              specialExperience.senderName
            }
            onClose={() =>
              setSpecialExperience(null)
            }
          />
        )}
      </AnimatePresence>
